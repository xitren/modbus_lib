...
public:
    ...
    {
        if (command_ == nullptr) {
            const auto st{in_data.msg().storage().begin()};
            const auto fn{in_data.msg().storage().begin() + in_data.msg().size()};
...
//            command_ = in_data.clone(&vault_);
    ...
    received() noexcept override
    {
        ...
        case master_state::waiting_reply:
            if (command_ != nullptr) {
                ...
                if (data<header>::deserialize(input_msg_.storage().begin()).slave_id
                    != ask_.slave) {
    ...
    read_input_bits(std::uint8_t slave, std::uint16_t address, std::array<bool, Size>& values)
    {
        ...
        if (!output_msg_.template serialize<header, request_fields_read, std::uint8_t, crc16ansi>(
                {{slave, static_cast<uint8_t>(function::read_discrete_inputs)},
                 {address, Size},
                 0,
                 nullptr})) {
            return exception::illegal_data_value;
    ...
    template <std::size_t Size>
    [[deprecated("Will be deleted. You should use command instead.")]] exception
    read_registers(std::uint8_t slave, std::uint16_t address,
                   std::array<std::uint16_t, Size>& values)
    {
        ...
        if (!output_msg_
                 .template serialize<header, request_fields_read, msb_t<std::uint16_t>, crc16ansi>(
        ...
        auto [pack, err] = input_msg<header, std::uint8_t, msb_t<std::uint16_t>>(slave);
        if ((error_ = err) != exception::no_error) [[unlikely]]
            return err;
    ...
    template <std::size_t Size>
    [[deprecated("Will be deleted. You should use command instead.")]] exception
    read_input_registers(std::uint8_t slave, std::uint16_t address,
                         std::array<std::uint16_t, Size>& values)
    {
        static_assert(Size < max_read_registers, "Too much to read!");
        if (slave == broadcast_address) [[unlikely]] {
    ...
    read_exception(std::uint8_t slave, std::uint8_t& val)
    {
        ...
        if (!output_msg_.template serialize<header, null_field, std::uint8_t, crc16ansi>(
                {{slave, static_cast<std::uint8_t>(function::read_exception_status)},
                 {},
                 0,
                 nullptr})) {
            return exception::illegal_data_value;
    ...
    write_bits(std::uint8_t slave, std::uint16_t address, const std::array<bool, Size>& vals)
    {
        ...
        const std::uint16_t coils_collect_num{static_cast<std::uint16_t>(
            (vals.size() % 8) ? (vals.size() / 8 + 1) : (vals.size() / 8))};
        ...
        if (!output_msg_
                 .template serialize<header, request_fields_wr_single, std::uint8_t, crc16ansi>(
                    ...
                    const std::array<std::uint16_t, Size>& vals)
    {
        ...
        if (!output_msg_.template serialize<header, request_fields_wr_single, msb_t<std::uint16_t>,
                                            crc16ansi>(
    ...
    [[deprecated("Will be deleted. You should use command instead.")]] exception
    mask_write_register(std::uint8_t slave, std::uint16_t address, std::uint16_t and_mask,
                        std::uint16_t or_mask)
    {
        ...
        ask_ = {slave, function::write_single_register};
        if (!output_msg_
                 .template serialize<header, request_fields_wr_mask, std::uint8_t, crc16ansi>(
                     {{slave, static_cast<uint8_t>(function::write_single_register)},
    ...
    write_and_read_registers(std::uint8_t slave, std::uint16_t write_address,
                             const std::array<std::uint16_t, WSize>& write_vals,
                             ...
                             std::array<std::uint16_t, RSize>&       read_vals)
    {
        ...
        const std::uint32_t max_write_address = write_address + WSize - 1;
        const std::uint32_t max_read_address  = read_address + RSize - 1;
        if ((max_write_address
             > static_cast<std::uint32_t>(std::numeric_limits<std::uint16_t>::max()))
            || (max_read_address > static_cast<std::uint32_t>(
                    std::numeric_limits<std::uint16_t>::max()))) [[unlikely]] {
            return exception::illegal_data_address;
        }
    ...
    template <std::size_t Size>
    [[deprecated("Will be deleted. You should use command instead.")]] exception
    read_fifo_queue(std::uint8_t slave, std::uint16_t address,
                    utils::circular_buffer<std::uint16_t, Size>& buffer)
    {
        if (slave == broadcast_address) [[unlikely]] {
            return exception::bad_slave;
        }
        ...
        if (!output_msg_.template serialize<header, msb_t<std::uint16_t>, std::uint16_t, crc16ansi>(
        ...
        auto [pack, err] = input_msg<header, request_fields_fifo, msb_t<std::uint16_t>>(slave);
        if ((error_ = err) != exception::no_error) [[unlikely]]
            return err;
        for (std::size_t i{}; i < pack.size; i++) {
...
private:
    ...
    wait_input_msg()
    {
        while ((state_ == master_state::waiting_reply)
               || (state_ == master_state::unrecoverable_error))
            ;
    ...
    request(modbus_master& master, std::uint8_t slave, diagnostics_sub_function sub,
            std::uint16_t& data)
    {
        master.ask_ = {slave, function::diagnostic};
        master.output_msg_
            .template serialize<header, msb_t<std::uint16_t>, msb_t<std::uint16_t>, crc16ansi>(
                {{slave, static_cast<uint8_t>(function::diagnostic)},
                 static_cast<uint16_t>(sub),
                 0,
                 nullptr});
        ...
        auto [pack, err]
            = master.input_msg<header, msb_t<std::uint16_t>, msb_t<std::uint16_t>>(slave);
...
struct modbus_master::diagnostic_module<diagnostics_sub_function::restart_comm_option> {
    static inline exception
    request(modbus_master& master, std::uint8_t slave, const std::uint16_t& data)
    {
        msb_t<std::uint16_t> data_formatted{data};
        master.ask_ = {slave, function::diagnostic};
        master.output_msg_
            .template serialize<header, msb_t<std::uint16_t>, msb_t<std::uint16_t>, crc16ansi>(
                {{slave, static_cast<uint8_t>(function::diagnostic)},
...
struct modbus_master::diagnostic_module<diagnostics_sub_function::force_listen_only_mode> {
    ...
    request(modbus_master& master, std::uint8_t slave)
    {
        master.ask_ = {slave, function::diagnostic};
        master.output_msg_
            .template serialize<header, msb_t<std::uint16_t>, std::uint16_t, crc16ansi>(
                {{slave, static_cast<uint8_t>(function::diagnostic)},
        ...
        auto [pack, err] = master.input_msg<header, msb_t<std::uint16_t>, std::uint16_t>(slave);
        if ((master.error_ = err) != exception::no_error) [[unlikely]]
            return err;
        return exception::no_error;
    }
