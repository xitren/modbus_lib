...
read_fifo(modbus_slave_base<TInputs, TCoils, TInputRegisters, THoldingRegisters, Fifo>& slave)
{
    ...
    using return_type =
        typename slave_type::msg_type::template fields_in<header, request_fields_fifo, msb_t<std::uint16_t>>;
    //=========Check parameters=====================================================================
    if (slave_type::request_type_fifo::length != slave.input().size()) {
        return exception::bad_data;
    }
    auto pack = slave.input().template deserialize_no_check<header, msb_t<std::uint16_t>, std::uint8_t, crc16ansi>();
    ...
    slave.output().template serialize<header, request_fields_fifo, msb_t<std::uint16_t>, crc16ansi>(data);
